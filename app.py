...
# =========================================
st.set_page_config(page_title="Klasifikasi Sampah - Kamera & Drive", page_icon="♻️")

# =========================================
# 2. DOWNLOAD & LOAD MODEL (SAFE MODE)
# =========================================
...
def load_full_system():
    ...
    # Solusi untuk Error: Unrecognized keyword arguments: ['batch_shape', 'optional']
    def fix_input_layer(config):
        config.pop('batch_shape', None)
        config.pop('optional', None)
        return InputLayer.from_config(config)

    # Memuat model dengan Custom Object Scope untuk menangani perbedaan versi Keras
    try:
...
st.write("Gunakan kamera HP/Laptop atau unggah file untuk deteksi sampah.")
...
# =========================================
# 4. PREPROSES & PREDIKSI
# =========================================
if img_input is not None and model is not None:
    # Konversi input ke format gambar PIL
    img = Image.open(img_input)
    
    # Tampilkan preview jika unggah file
    if option == "Unggah Gambar":
        st.image(img, caption='Gambar Terpilih', use_container_width=True)

    if st.button('Mulai Deteksi'):
        with st.spinner('Sedang menganalisis...'):
            # Preprocessing sesuai spesifikasi training (150x150)
            img_array = np.array(img.convert('RGB')) # Pastikan RGB
            ...
            st.write("---")
            # Mapping sesuai training: 0 = Organik, 1 = Anorganik
            if score > 0.5:
                st.error(f"### HASIL: ANORGANIK (RECYCLABLE)")
